...
def test_list_saves_delegates(router, mock_client):
    mock_client.request.return_value = [{"id": 1}]
    result = router.list_saves(5)
    assert result == [{"id": 1}]


def test_upload_save_delegates(router, mock_client):
    mock_client.upload_multipart.return_value = {"id": 1}
    result = router.upload_save(5, "/tmp/test.srm", "retroarch")
    assert result == {"id": 1}


def test_get_save_metadata_delegates(router, mock_client):
    mock_client.request.return_value = {"id": 10}
    result = router.get_save_metadata(10)
    assert result == {"id": 10}


def test_get_rom_detail_delegates(router, mock_client):
    mock_client.request.return_value = {"id": 5, "name": "Test ROM"}
    result = router.get_rom_detail(5)
    assert result == {"id": 5, "name": "Test ROM"}


def test_create_note_delegates(router, mock_client):
    mock_client.post_json.return_value = {"id": 1}
    result = router.create_note(5, {"body": "test"})
    assert result == {"id": 1}


def test_update_note_delegates(router, mock_client):
    mock_client.put_json.return_value = {"id": 1}
    result = router.update_note(5, 1, {"body": "updated"})
    assert result == {"id": 1}
